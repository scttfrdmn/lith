...
func newMountCmd() *cobra.Command {
	...
	fl.StringVar(&f.partsMax, "parts-max", "auto", "fetch files at or below this size whole as concurrent block-sized range parts, once the handle's reads prove they tile (#229) — never at open, so a hyperslab or footer probe is served precise instead (0 disables; a single GET below one block). \"auto\" derives it from NIC baseline × first-byte latency, clamped to [--small-file, 64MiB]: a fat pipe fetches whole cheaply, a thin pipe keeps sub-file reads byte-exact (#220)")
