#!/usr/bin/env bash
# Session-37 bootstrap: after=framecache-failclosed, before=main: lith v0.3.1 (main) + lith-main, cargoship v0.24.5,
...
if ! command -v cargoship >/dev/null; then cd /tmp && curl -sSL "https://github.com/scttfrdmn/cargoship/releases/download/v0.24.5/cargoship_0.24.5_linux_arm64.tar.gz" -o cs.tgz && tar xzf cs.tgz cargoship && sudo mv cargoship /usr/local/bin/; fi
...
git checkout -q main && go build -o bin/lith-main ./cmd/lith
git checkout -q framecache-failclosed && go build -o bin/lith ./cmd/lith
