...
func TestRandomDispatchesNothing(t *testing.T) {
	...
	seq := []int64{0, 7, 2, 9, 1}
	for i, b := range seq {
		if got := p.Observe(b, 0, 0, 0); len(got) != 0 && i > 0 {
...
func TestReReadSameBlockNoop(t *testing.T) {
	...
	p.Open()
	p.Observe(0, 0, 0, 0)
	if got := p.Observe(0, 0, 0, 0); got != nil {
...
func TestDoubleSeekRandomThenRecover(t *testing.T) {
	...
	p.Observe(200, 0, 0, 0) // first seek
	p.Observe(400, 0, 0, 0) // second seek, no progress between
	if p.State() != Random {
