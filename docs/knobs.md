...
| `--footer-tier2` | `true` | For footer-family containers (Parquet/ORC/Arrow/zip), prefetch the index-resolved projection — a touched Parquet row group's projected column chunks, or a read zip entry + the next few in directory order (#108). Tier 1 (footer + head prefetch on open) always runs. **Bytes vs latency:** on a scattered Parquet projection lith fetches whole 1 MiB chunks, so it reads more bytes than a byte-exact reader (pyarrow) but coalesces into far fewer GETs — a latency win where round-trips dominate, not a byte win (byte-exact fetch is [#118](https://github.com/scttfrdmn/lith/issues/118)). `false` leaves tier 1 only. |
